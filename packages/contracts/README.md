...
This folder contains the Solidity smart contracts for the IDCom platform, built with Foundry. This package is part of the IDCom monorepo and uses npm workspaces for dependency management.
...
**Note:** This monorepo uses npm (not pnpm) for package management. All scripts use `npm` or `npx` commands.
...
# Then deploy from contracts directory
cd packages/contracts
./script/deploy-proof-of-human.sh
```
